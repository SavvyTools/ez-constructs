...
export class SimpleCodebuildProject extends EzConstruct {
  ...
  private createWebHookFilters(base?: string, gitEvent?: GitEvent, branches?:Array<string>, githubUserIds?: number[]): FilterGroup[] | undefined {
    ...
    // Create consolidated user pattern if users are specified
    const userPattern = githubUserIds && githubUserIds.length > 0 
      ? githubUserIds.join('|')
      : undefined;

    // Helper function to create a single filter group with all conditions
    const createFilter = (baseEvents: EventAction[], isForBranch: boolean = false, branchName?: string) => {
      ...
      // Add branch reference condition
      if (base && !isForBranch) {
    ...
    // Handle push events for base branch
